...
## Open issues
- **OpenRouter key on Vercel.** `OPENROUTER_API_KEY` on `lodestar-command` is not the paid-account key (that key shows no production use); its own credit limit blocks Compare (HTTP 402). Paste the paid key into the Vercel variable (or raise that key's limit on openrouter.ai → Keys), then redeploy. Briefs cost about $0.007, drafts about $0.0001; Compare (Opus) more.
